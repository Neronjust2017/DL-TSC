...
input_node = ak.Input()
output_node = ak.TemporalConvNetBlock()(input_node)
output_node = ak.ClassificationHead()(output_node)
clf = ak.AutoModel(inputs=input_node, outputs=output_node, max_trials=20,
                   name='UCI_tcn_Greedy',directory='nas_result')
#
sys.stdout = Logger('nas_result/UCI_tcn_Greedy/log', sys.stdout)
sys.stderr = Logger('nas_result/UCI_tcn_Greedy/log_file', sys.stderr)		# redirect std err, if necessary
...
loss = clf.evaluate(final_testset[0], final_testtarget)
y_pred = clf.predict(final_testtarget[0])
accuracy = accuracy_score(final_testtarget, y_pred)
precision = precision_score(final_testtarget, y_pred, average='macro')
recall  = recall_score(final_testtarget, y_pred, average='macro')
f1 =f1_score(final_testtarget, y_pred, average='macro')
...
model.save('nas_result/UCI_tcn_Greedy/bestmodel.h5')
