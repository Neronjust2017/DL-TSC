...
class HyperXception1D(hypermodel.HyperModel):
    ...
    def __init__(self,
                 ...
                 classes=None,
                 **kwargs):
        super(HyperXception1D, self).__init__(**kwargs)
    ...
    def build(self, hp):
        ...
        # Initial conv1d.
        conv1d_num_filters = hp.Choice(
            'conv1d_num_filters', [16, 32, 64, 128], default=32)
        ...
        sep_num_filters = hp.Int(
            'sep_num_filters', 32, 156, step=32, default=64)
...
def sep_conv(x, num_filters, kernel_size=3, activation='relu'):
    if activation == 'selu':
        x = layers.SeparableConv1D(num_filters, kernel_size,
                                   activation='selu',
                                   padding='same',
...
def residual(x, num_filters,
    ...
    "Residual block."
    if max_pooling:
        res = layers.Conv1D(num_filters, kernel_size=1, strides=pool_strides, padding='same')(x)
...
def conv(x, num_filters,
         kernel_size=3, activation='relu', strides=2):
