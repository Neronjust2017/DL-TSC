...
from kerastuner.applications import resnet_v2
from kerastuner.applications import xception1d
from kerastuner.applications import resnet_v2
from kerastuner.applications import inceptiontime
from kerastuner.applications import temporalconvnet
...
class RNNBlock(base.Block):
    ...
    def build(self, hp, inputs=None):
        ...
        return_sequences_last = self.return_sequences
        if self.return_sequences is None:
            return_sequences_last = hp.Boolean('return_sequences', default=False)
...
class Xception1dBlock(base.Block, xception1d.HyperXception1D):
    """XceptionBlock.
...
class InceptionTimeBlock(base.Block, inceptiontime.Hyperinceptiontime):
    """Block for InceptionTime.

    # Arguments
        type: String. 'v1', 'v2'. The type of InceptionTime to use.
            If left unspecified, it will be tuned automatically.
        depth: Int. 6, 9 , 12 . 
            If left unspecified, it will be tuned automatically.
    """
    ...
    def build(self, hp, inputs=None):
        ...
        hp.Choice('type', ['v1', 'v2'], default='v1')
        hp.Choice('depth', [6, 9, 12], default=6)

        set_hp_value(hp, 'type', self.type)
        set_hp_value(hp, 'depth', self.depth)

        model = super().build(hp)
        return model.outputs

class ResNetV2Block(base.Block, resnet_v2.HyperResNetV2):
    """Block for ResNetV2.

    # Arguments
        type: String.  The type of InceptionTime to use.
            If left unspecified, it will be tuned automatically.
        depth: Int.
            If left unspecified, it will be tuned automatically.
    """
    ...
    def build(self, hp, inputs=None):
        ...
        hp.Choice('type', ['resnet34', 'resnet34_with_L2', 'resnet34_se', 'resnet34_BNGammaZero', 'resnet34_BN2'],
                  default='resnet34')
        hp.Choice('depth', [7, 15, 23, 31], default=15)
...
class TemporalConvNetBlock(base.Block, temporalconvnet.HyperTemporalConvNet):
    """Block for TemporalConvNet.
    ...
    def build(self, hp, inputs=None):
        ...
        hp.Choice('num_levels', [2, 3, 4], default=2)

        set_hp_value(hp, 'num_levels', self.num_levels)
