...
def interpolation(input_tensor, ref_tensor, name):  # resizes input_tensor wrt. ref_tensor
    ...
    return tf.compat.v1.image.resize_nearest_neighbor(input_tensor, [H, W], name=name)
# tf.compat.v1.image.resize_nearest_neighbor(input_tensor, [H.value, W.value], name=name)
...
data = np.random.randint(1,20,size=(10,224,224,3))
label = np.random.randint(0,1,size=(10,1,1,1000))
model.fit(data,label, callbacks=[tbCallBack])